...
Dharma is a protocol that enables decentralized issuance and administration of tokenized debt.  The protocol creates a common informational interface so exchanges, brokerages, issuers and investors can facilitate trading and pricing of tokenized debt without having to rely on a singular centralized entity.  An agent-based model creates two classes of service providers, underwriters and relayers, who compete in distinct marketplaces for incentives from fees.  The underwriters are trusted originators who can use their reputation to reduce credit risk by enriching any offering with certified meta data.  The relayers facilitate the issuance and funding of debt in a trust-less manner.  Optionally, both can be empirically evaluated on historical performance.  Dharma debt issuance process requires a sole on-chain transaction and is inspired by the 0x Protocol.</div>
